...
module.exports = function() {
    ...
    **/
    this.getActionQuantity = function(player_id, ability) {
        return new Promise(res => {
            sql("SELECT * FROM action_quantities WHERE player_id= " + connection.escape(player_id) + " AND ability_hash=" + hash(ability), result => {
                if(!result[0]) res(0);
    ...
    **/
    this.initActionQuantity = function(player_id, ability) {
        return new Promise(res => {
            sql("INSERT INTO action_quantities (player_id,ability_hash,quantity) VALUES (" + connection.escape(player_id) + "," + hash(ability) + ",1)", result => {
                 res();
            });
        });
    }
    ...
    **/
    this.increaseActionQuantity = function(player_id, ability) {
        return new Promise(res => {
            sql("UPDATE action_quantities SET quantity=quantity+1 WHERE player_id= " + connection.escape(player_id) + " AND ability_hash=" + hash(ability), result => {
                 res();
            });
        });
    }    
