...
module.exports = function() {
    ...
    **/
    this.groupsDisband = async function(group, id = 0, forceDisband = false) {
        let result = await sqlPromEsc("SELECT * FROM active_groups WHERE disbanded=0 AND name=" + connection.escape(group) + " OR channel_id=", id);
    ...
    **/
    this.groupsReopen = async function(group, id = 0) {
        let result = await sqlPromEsc("SELECT * FROM active_groups WHERE disbanded=1 AND name=" + connection.escape(group) + " OR channel_id=", id);
        let groupChannel = await mainGuild.channels.fetch(result[0].channel_id);
        // send reopen message
