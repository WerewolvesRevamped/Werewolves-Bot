...
client.on("messageCreate", async message => {
    ...
    case "fortune":
        ...
        let index = (Math.floor((+ new Date())/(1000*60*60*4)) + (+message.member.id.substr(0,8)) + message.member.displayName.length) % 44;
        if(isGameMaster(message.member) && args[0] && args[0] === "r") index = Math.floor(Math.random() * 44);
        if(index >= 22) index += 9;
