...
module.exports = function() {   
        ...
        if(!urlPack) {
            ...
            if(!urlExists) {
                ...
                if(lutval) { // check lut
                    ...
                    let classesWithPlaceholders = ["townsfolk","werewolf","unaligned","solo","mafia","neutral"]; // list of classes with a specific placeholder icon
    ...
    **/
    this.getRoleTypeData = function(roleTypeIn) {
        ...
        switch(roleTypeIn.trim()) {
            ...
            case "archive":
            case "archived": roleTypeID = 9; roleType = "Deleted Role"; break;
        }
    ...
    **/
    this.verifyInfoMessage = function(input) {
		return verifyRole(input) || verifyInfo(input) || verifyGroup(input) || verifyLocationName(input) || verifyTeam(input) || verifyTeamName(input);
	}
