    ...
    **/
    ...
    this.COMMANDS = [
        ...
        ["schedule", PERM.GM, "Manages schedule.", "<Subcommand>", "Group of commands to handle schedule. Use $help schedule <subcommand> for detailed help. If used without a subcommand runs $schedule list", "", [], CMDSTATE.RDY],
            ...
            ["schedule add", PERM.GM, "Adds a scheduled event.", "<Event Type> <Timestamp> <Event Value(s)>", "Schedules an event. <Timestamp> may be provided in the following formats: HH:MM, <N>s, <N>m, <N>h, <N>d or unix timestamp. Use $help schedule add <Event Type> for more info on a specific event type.", "schedule add command 5m help", ["✅ Command scheduled for in 5 minutes!"], CMDSTATE.RDY],
                ["schedule add command", PERM.GM, "Adds a scheduled command.", "<Timestamp> <Command>", "Schedules a command. Specify a command as <Command>, without the prefix.", "schedule add command 5m help", ["✅ Command scheduled for in 5 minutes!"], CMDSTATE.RDY, ["schedule add cmd"]],
            ...
            ["schedule clear", PERM.GM, "Clears the schedule.", "", "Clears the entire schedule.", "schedule clear", ["✅ Cleared schedule!"], CMDSTATE.RDY],
        /** CC Module **/
        // CC
        ...
        ["booster", PERM.AL, "Manages your boosters.", "", "Group of commands to manage your boosters. See $help booster <subcommand>", "", [], CMDSTATE.RDY],
            ["booster use", PERM.AL, "Uses a booster.", "<Booster Code>", "Uses a booster.", "booster use bst:xp150", [], CMDSTATE.RDY],
            ["booster list", PERM.AL, "Lists your boosters.", "", "Lists your boosters.", "booster list", [], CMDSTATE.RDY],
        ...
        ["curse", PERM.AL, "Manages your boosters.", "", "Group of commands to manage your curses. See $help booster <subcommand>", "", [], CMDSTATE.RDY],
            ["curse use", PERM.AL, "Uses a curse.", "<Booster Code> <Target>", "Uses a curse.", "curse use cur:pack6 @Ts", [], CMDSTATE.RDY],
            ["curse list", PERM.AL, "Lists your curses.", "", "Lists your curses.", "curse list", [], CMDSTATE.RDY],
            ["curse active", PERM.AL, "Lists currently active curses.", "", "Lists all currently active curses applied to you.", "curse active", [], CMDSTATE.RDY],
        ...
        ["list_ghost", PERM.AL, "Lists ghostly players.", "", "Lists all ghostly players", "ghost", ["Ghostly Players | Total: 3","🛠 - McTsts (@McTsts)","🏹 - venomousbirds  (@venomousbirds)","🐼 - Jordan (@kruthers)"], CMDSTATE.RDY],
        ...
        ["emojis_alive", PERM.AL, "Provides an emoji & player ID (alive only) list.", "", "Gives you a list of emojis and player ids as well as a list of all emojis. Can be used for CC creation. List living players only.", "emojis_alive", ["🛠 242983689921888256","🦎 458727748504911884","🏹 277156693765390337","🛠 🦎 🏹"], CMDSTATE.RDY],
        // Sign Up Commands
        ...
        ["options list", PERM.GM, "Help Page - Options", "", availableStats.filter(el => el.cmd && el.desc).slice(0, 40).map(el => el.cmd + ": " + el.desc).join("\n") + "\nContinued in $help options list2", "", [""], CMDSTATE.RDY, ["optionslist"]],
        ["options list2", PERM.GM, "Help Page - Options (Page 2)", "", availableStats.filter(el => el.cmd && el.desc).slice(40, 80).map(el => el.cmd + ": " + el.desc).join("\n"), "", [""], CMDSTATE.RDY],
        ...
        ["options cc_rule", PERM.GM, "Help Page - Options - CC Rule", "<Rule Name>", "A complicated option, so it gets a dedicated help page. Determines how to automatically increment ccs. Set it to the name of the rule you want to use:\nnone: no automatic cc increase\nnight: cc increase at end of night\nday: cc increase at end of day\nphase: cc increase at end of every phase\ndeath: cc increase on every death", "options cc_rule night", [""], CMDSTATE.RDY , ["optionscc","options_cc"]],
        // gamephase
        ...
        ["apply", PERM.GM, "Applies an attribute.", "<Target Player> <Attribute>", "Applies an attribute to a specified player.", `apply 242983689921888256 ghostly`, [], CMDSTATE.RDY],
        ...
        ["attributes", PERM.GM, "Manages attributes.", "<Subcommand>", "Group of commands to handle attributes. $help attributes <sub-command> for detailed help.", "", [], CMDSTATE.RDY],
            ...
            ["attributes edit", PERM.GM, "Edits an active attribute.", "<Attribute ID> <Column> <Value>", "Edits a single active attribute by ID. Use $attributes active to view IDs. Use $attributes view to see detailed attribute info.", "attributes edit 1 attr_type custom", ["✅ Updated active attribute instance."], CMDSTATE.RDY],
            ["attributes view", PERM.GM, "Views an active attribute.", "<Attribute ID>", "Views a single active attribute by ID. Use $attributes active to view IDs.", "attributes view 1", ["✅ Viewing attribute:","ai_id: 1","attr_type: custom", "owner: 242983689921888256", "etc"], CMDSTATE.RDY],
        /** Game Module **/
        // dr
        ...
        ["restart", PERM.GM, "Restarts a game.", "", "Restarts the game (only use during D0, all players must be alive, roles may be updated prior to using this command).", "restart", [], CMDSTATE.RDY],
        ["reset_debug", PERM.GM, "Resets a game (Debug Mode).", "", "Resets a game 'in place', keeping all players and SCs, but resetting everything else. To be precise the following things differ from a normal reset:\n- all player properties are reset to default, but players remain signedup.\n- all connections are kept (otherwise player scs cannot be found anymore).\n- discord roles are not updated besides for removal of DRs - that means dead players will not regain participant role.\n- nicknames are not reset.\n- public, secret and conspiracy channels are kept, however some secret channels (such as group channels) lose functionality and new ones may be generated.\n- the game is reset to D0 and starts again.", "reset_debug", [], CMDSTATE.RDY],
        ...
        ["reevaluate", PERM.GM, "Reevaluates win conditions.", "", "Reevaluates win conditions of all teams. Use this when a game is meant to end but did not end (e.g. due to a death outside of phase change).", "reevaluate", [], CMDSTATE.RDY],
        ["archived", PERM.GM, "Marks game as archived.", "", "Marks a game as archived allowing it to be reset.", "archived", [], CMDSTATE.RDY],
        ...
        // alias
        ["alias", PERM.GM, "Manages aliases.", "<Subcommand>", "Group of commands to handle alias. $help alias <sub-command> for detailed help.", "", [], CMDSTATE.UNK],
            ["alias set", PERM.GM, "Creates a new alias.", "<Alias> <Target>", "Creates an alias that redirect <Alias> to <Target>", "alias add villager citizen", [], CMDSTATE.UNK, ["alias add"]],
            ["alias remove", PERM.GM, "Deletes a alias.", "<Alias>", "Deletes a specific aliases.", "alias remove villager", [], CMDSTATE.UNK],
            ["alias clear", PERM.GM, "Deletes all aliases.", "", "Deletes all existing aliases.", "alias clear", [], CMDSTATE.UNK],
            ["alias list", PERM.GM, "Lists all aliases.", "", "Lists all existing aliases, sorted by their target.", "alias list", [], CMDSTATE.RDY],
        // info commands
