    ...
    **/
	this.confirmActionExecute = function(command, message, messageSent) {
		switch(command) {
			...
			case "restart": cmdRestart(message.channel); break;
			...
			case "players log5": cmdPlayersLog(message.channel, "-"); break;
			case "players log6": cmdPlayersLog3(message.channel, "-"); break;
    ...
    **/
    function getWarning(action) {
        switch(action) {
			...
			case "restart":
                return " **WARNING:** This is an irreversible destructive action that deletes a large amount of data. You are restarting **__"  + stats.game + "__**. Are you __absolutely certain__ you want to perform this action?";
			case "end":
